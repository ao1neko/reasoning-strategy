...
CUDA_VISIBLE_DEVICES=0 PYTHONHASHSEED=0 python "${project_dir}/src/main.py" \
        ...
        --model_name=bart \
        --load_model_dir="${save_model_dir}/reasoning_model/42/pretrain/bart_large/no_reasoning_step/best_model" \
        --train=false \
        --predict=true \
        --train_epochs=1 \
        --eval_steps=1  \
        --output_dir="${save_model_dir}/reasoning_model/42/pretrain/bart_large/no_reasoning_step_test" \
        --run_dir="42/pretrain/bart_large/no_reasoning_step_test" \
        --batch_size=64 \
